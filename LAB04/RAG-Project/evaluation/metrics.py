...
def hit_at_k(retrieved, relevant, k):
    """1.0 if a correct chunk is in the top-k"""
    return 1.0 if set(relevant) & set(retrieved[:k]) else 0.0


def recall_at_k(retrieved, relevant, k):
    """Proportion of all correct chunks found in the top-k"""
    if not relevant:
        return 0.0
...
def precision_at_k(retrieved, relevant, k):
    """Proportion of the top-k that are correct chunks"""
    return len(set(relevant) & set(retrieved[:k])) / k if k else 0.0


def reciprocal_rank(retrieved, relevant):
    """1/rank of the first correct chunk (0 if not found)"""
...
def ndcg_at_k(retrieved, relevant, k):
    """DCG divided by ideal DCG — comparable across questions"""
...
def average(rows):
    """Average across queries (equal weight for each query)"""
    if not rows:
        return {}
    return {
...
def print_table(results, metrics):
    """Display {experiment_name: scores} as a comparison table"""
    if not results:
        return
...
if __name__ == "__main__":
    # python -m evaluation.metrics — Check formulas with pre-calculated values
